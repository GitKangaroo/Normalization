...
namespace AnaIO
{
  ...
  // Declare histograms
  TH1D * hRecoB0_M = 0x0;
  TH1D * hRecoDsstar_M = 0x0;
  TH1D * hRecoDs_M = 0x0;
  TH1D * hRecoSoftPhoton_PT = 0x0;
  TH1D * hRecoSoftPion_PT = 0x0;
  TH1D * hDsIPCHI2 = 0x0;
  TH1D * hPiMinusIPCHI2 = 0x0;
  TH1D * hDsIPCHI2_OWNPV = 0x0;
  TH1D * hRecoPiMinus_PT = 0x0;
  TH1D * hRecoB0Chi2DOF = 0x0;
  TH1D * hRecoB0IP = 0x0;

  TH1D * hRecoDs_M_Selected = 0x0;
  TH1D * hRecoDs_PT_Selected = 0x0;
  TH1D * hRecoDs_IPChi2 = 0x0;
  TH1D * hRecoDsstar_PT_Selected = 0x0;
  TH1D * hRecoB0_PT_Selected = 0x0;
  TH1D * hRecoDs_M_Selected_Resonance = 0x0;
  TH1D * hnCandidate = 0x0;

  TH2D * hSoftphotonPtVSConeR2 = 0x0;
  TH2D * hDsDalitzPlotKKVSKPi = 0x0;
  TH2D * hDsDalitzPlotKKVSKPi_Selected = 0x0;
  
  TH1D * hRecoDsstar_M_Selected = 0x0;
  TH1D * hRecoB0_M_Selected = 0x0;
  TH1D * hRecoDsstar_M_SelectedFinal = 0x0;

  TTree * GetInputTree(TFile * fin, const TString tname, const TString tag)
  {
    TTree * fChain=(TTree*) fin->Get(tname);
      if(!fChain){
        cout<<"no tree!"<<endl;
        gDirectory->ls();
        exit(1);
  ...
  void IniHist(TList * lout)
  {
 
    hRecoB0_M = new TH1D("a001hRecoB0_M", ";#it{m}(B^{0}) [MeV/c^{2}];Candidates", 30, 4800, 6200);
    lout->Add(hRecoB0_M);
    hRecoDsstar_M = new TH1D("a002hRecoDsstar_M", ";#it{m}(D^{* +}_{s}) [MeV/c^{2}];Candidates", 30, 2000, 2200);
    lout->Add(hRecoDsstar_M);
    hRecoDs_M = new TH1D("a003hRecoDs_M", ";#it{m}(D^{+}_{s}) [MeV/c^{2}];Candidates", 30, 1940, 2000);
    lout->Add(hRecoDs_M);

    hRecoSoftPhoton_PT = new TH1D("a004hRecoSoftPhoton_PT", ";#it{p}_{T} #gamma_{soft} [MeV/c];Candidates", 30, 0, 2000);
    lout->Add(hRecoSoftPhoton_PT);
    hRecoSoftPion_PT = new TH1D("a005hRecoSoftPion_PT", ";#it{p}_{T} #pi_{soft} [MeV/c];Candidates", 30, 0, 15000);
    lout->Add(hRecoSoftPion_PT);
    hDsIPCHI2 = new TH1D("a006hDsIPCHI2", ";ln(D^{+}_{s} IP#chi^{2} (PV)); Candidates", 30, 0, 15);
    lout->Add(hDsIPCHI2);
    hPiMinusIPCHI2 = new TH1D("a007hPiMinusIPCHI2", ";ln(#pi^{-}_{soft} IP#chi^{2} (PV)); Candidates", 30, 0, 15);
    lout->Add(hPiMinusIPCHI2);
    hDsIPCHI2_OWNPV = new TH1D("a008hDsIPCHI2_OWNPV", ";D^{+}_{s} IP#chi^{2} (PV); Candidates", 30, 0, 15);
    lout->Add(hDsIPCHI2_OWNPV); 
    hRecoPiMinus_PT = new TH1D("a009hRecoPiMinus_PT", ";#pi^{-} #it{p}_{T} [MeV/c]; Candidates", 30, 0, 9000);
    lout->Add(hRecoPiMinus_PT); 
    hRecoB0Chi2DOF = new TH1D("a010hRecoB0Chi2DOF", ";B^{0} #chi^{2}/DOF ; Candidates", 30, 0, 12);
    lout->Add(hRecoB0Chi2DOF); 
    hRecoB0IP = new TH1D("a011hRecoB0IPF", ";B^{0} IP [mm] ; Candidates", 30, 0, 0.2);
    lout->Add(hRecoB0IP); 

    hRecoDs_M_Selected = new TH1D("a012hRecoDs_M_Selected", ";#it{m}(D^{+}_{s}) [MeV/c^{2}];Candidates", 30, 1940, 2000);
    lout->Add(hRecoDs_M_Selected);
    hRecoDs_PT_Selected = new TH1D("a013hRecoDs_PT_Selected", ";#it{p}_{T}(D^{+}_{s}) [MeV/c];Candidates", 30, 0, 16000);
    lout->Add(hRecoDs_PT_Selected);
    hRecoDs_IPChi2 = new TH1D("a014hRecoDs_IPChi2", ";IP #chi^{2}(D^{+}_{s}) [MeV/c];Candidates", 30, 0, 1000);
    lout->Add(hRecoDs_IPChi2);

    hRecoDsstar_PT_Selected = new TH1D("a015hRecoDsstar_PT_Selected", ";#it{p}_{T}(D^{+ *}_{s}) [MeV/c];Candidates", 30, 0, 16000);
    lout->Add(hRecoDsstar_PT_Selected);
    hRecoB0_PT_Selected = new TH1D("a016hRecoB0_PT_Selected", ";#it{p}_{T}(B^{0}) [MeV/c];Candidates", 30, 0, 16000);
    lout->Add(hRecoB0_PT_Selected);
    hRecoDs_M_Selected_Resonance = new TH1D("a017hRecoDs_M_Selected_Resonance", ";#it{m}(D^{+}_{s}) [MeV/c^{2}];Candidates", 30, 1940, 2000);
    lout->Add(hRecoDs_M_Selected_Resonance);
    hnCandidate = new TH1D("a018hnCandidate", ";nCandidate;Candidates", 20, 0, 20);
    lout->Add(hnCandidate);

    hSoftphotonPtVSConeR2 = new TH2D("b001hSoftphotonPtVSConeR2", ";p_{T} #gamma_{soft} [MeV/c^{2}];r^{2}", 30, 0, 1500, 30, 0, 1.5);
    lout->Add(hSoftphotonPtVSConeR2);
    hDsDalitzPlotKKVSKPi = new TH2D("b002hDsDalitzPlotKKVSKPi", ";#it{m}(K^{-}K^{+})^{2} [MeV^{2}];#it{m}(#pi^{-}K^{+})^{2} [MeV^{2}]", 60, 800000, 4000000, 60, 300000, 2500000);
    lout->Add(hDsDalitzPlotKKVSKPi);
    hDsDalitzPlotKKVSKPi_Selected = new TH2D("b003hDsDalitzPlotKKVSKPi_Selected", ";#it{m}(K^{-}K^{+})^{2} [MeV^{2}];#it{m}(#pi^{-}K^{+})^{2} [MeV^{2}]", 60, 800000, 4000000, 60, 300000, 2500000);
    lout->Add(hDsDalitzPlotKKVSKPi_Selected);

    hRecoDsstar_M_Selected = new TH1D("c001hRecoDsstar_M_Selected", ";#it{m}(D^{* +}_{s}) [MeV/c^{2}];Candidates", 25, 2000, 2200);
    lout->Add(hRecoDsstar_M_Selected);
    hRecoB0_M_Selected = new TH1D("c002hRecoB0_M_Selected", ";#it{m}(B^{0}) [MeV/c^{2}];Candidates", 20, 5200, 5500);
    lout->Add(hRecoB0_M_Selected); 
    hRecoDsstar_M_SelectedFinal = new TH1D("c003hRecoDsstar_M_SelectedFinal", ";#it{m}(D^{* +}_{s}) [MeV/c^{2}];Candidates", 30, 2000, 2200);
    lout->Add(hRecoDsstar_M_SelectedFinal);
